...
var self = (module.exports = {
  ...
  proxy: function (req, res) {
    ...
    var request = unirest[req.method.toLowerCase()](
      req.connection.kong_admin_url + req.url
    );
    ...
    switch (req.method.toLowerCase()) {
      case 'patch':
        return ProxyHooks.beforeEntityUpdate(
          entity,
          req.param('id'),
          req.connection.id,
          _.merge(req.body, { extras: konga_extras }),
          function (err, data) {
  ...
  listProxy: (req, res) => {
    ...
    sails.log.debug(
      'KongProxyController:listAllEntityRecords:req.method',
      req.method
    );
    sails.log.debug(
      'KongProxyController:listAllEntityRecords:req.url',
      req.url
    );
  ...
  send: function (entity, unirestReq, konga_extras, req, res) {
    ...
    unirestReq.end(function (response) {
      ...
      switch (req.method.toLowerCase()) {
        case 'get':
          return ProxyHooks.afterEntityRetrieve(
            entity,
            req,
            response.body,
            function (err, data) {
        ...
        case 'post':
          return ProxyHooks.afterEntityCreate(
            entity,
            req,
            response.body,
            konga_extras || {},
