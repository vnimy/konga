...
var KongPluginService = _.merge(_.cloneDeep(require('./KongService')), {
  makeSelfSignedCerts: function makeSelfSignedCerts(cb) {
    ...
    var cmd =
      'cd ' + cert_path + ' && openssl genrsa -out certificate.key 2048';
    var key;
    var cert;
    exec(cmd, function (error, stdout, stderr) {
      key = fs.readFileSync(
        path.join(__dirname, '..', '..', '.tmp/certificate.key')
      );
      fds.push(key);
      cmd =
        'cd ' +
        cert_path +
        ' && openssl req -new -x509 -key certificate.key -out certificate.cert -days 3650 -subj /CN=cert';
      exec(cmd, function (error, stdout, stderr) {
        cert = fs.readFileSync(
          path.join(__dirname, '..', '..', '.tmp/certificate.cert')
        );
        fds.push(cert);
        cb(null, fds);
      });
  ...
  addDynamicSSLPlugin: function (fds, req, res) {
    return unirest
      .post(req.connection.kong_admin_url + req.url.replace('/kong', ''))
      .headers({ 'Content-Type': 'multipart/form-data' })
      .field('name', req.body.name)
      .field('config.only_https', req.body['config.only_https'] || false)
      .field(
        'config.accept_http_if_already_terminated',
        req.body['config.accept_http_if_already_terminated'] || false
      )
  ...
  addCertificates: function (fds, req, res) {
    var request = unirest.post(
      req.connection.kong_admin_url + req.url.replace('/kong', '')
    );
  ...
  retrieve: function (req, res) {
    unirest
      .get(req.connection.kong_admin_url + req.url.replace('/kong', ''))
      .end(function (response) {
        if (response.error) return res.kongError(response);
        return res.json(response.body);
      });
  ...
  richList: function (req, res) {
    var _this = this;
    unirest
      .get(req.connection.kong_admin_url + '/plugins/enabled')
      .end(function (response) {
        if (response.error) return res.kongError(response);
  ...
  update: function (req, res) {
    unirest
      .patch(req.connection.kong_admin_url + req.url.replace('/kong', ''))
      .send(req.body)
      .end(function (response) {
        if (response.error) return res.kongError(response);
        return res.json(response.body);
  ...
  updateOrCreate: function (req, res) {
    unirest
      .put(req.connection.kong_admin_url + req.url.replace('/kong', ''))
      .send(req.body)
      .end(function (response) {
        if (response.error) return res.kongError(response);
        return res.json(response.body);
  ...
  delete: function (req, res) {
    unirest
      .delete(req.connection.kong_admin_url + req.url.replace('/kong', ''))
      .end(function (response) {
        if (response.error) return res.kongError(response);
        return res.json(response.body);
      });
  ...
  groups: function () {
    return [
      ...
      },
      {
        ...
        description: '管理、节流和限制入站和出站API流量',
        plugins: {
          'rate-limiting': {
            description:
      ...
      },
      {
        ...
        hasConsumerPlugins: true,
        icon: 'mdi-chart-bar',
        description: '可视化、检查和监控API和微服务流量',
        plugins: {
          galileo: {
      ...
      },
      {
        ...
        description: '使用日志交换组件来记录请求和响应数据',
        plugins: {
          'tcp-log': {
          ...
          filters: {
            removeSelected: function (values) {
              return function (item) {
                if (values instanceof Array) return values.indexOf(item) < 0;
                else return values;
              };
            },
          },
