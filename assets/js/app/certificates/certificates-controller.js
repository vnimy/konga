...
(function () {
  ...
  angular.module('frontend.certificates').controller('CertificatesController', [
    '$scope',
    '$rootScope',
    '$log',
    '$state',
    ...
    function controller(
      ...
      $scope = angular.extend(
        $scope,
        angular.copy(ListConfig.getConfig('certificate', CertificateModel))
      );
          ...
          controller: function (
            ...
            $scope.onTagInputKeyPress = function ($event) {
              if ($event.keyCode === 13) {
                ...
                $scope.data.tags = $scope.data.tags.concat(
            ...
            $scope.submitCerts = function () {
              ...
              if (!data.cert || !data.key) {
                $scope.errorMessage = '证书的公钥和私钥不能为空';
                return false;
              }
      ...
      function _fetchData() {
        ...
        CertificateModel.load({
          ...
          if (response.data && Object.keys(response.data).length) {
            $scope.certificates =
              Semver.cmp($rootScope.Gateway.version, '0.10.1') > 0
                ? response.data.data
                : response.data;
          } else {
            $scope.certificates = [];
