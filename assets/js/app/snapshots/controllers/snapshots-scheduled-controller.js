    ...
    .controller('SnapshotsScheduledController', [
      ...
      function controller(
        ...
        $scope = angular.extend(
          $scope,
          angular.copy(
            ListConfig.getConfig('snapshotschedule', SnapshotSchedule)
          )
        );
        ...
        function _fetchData() {
          ...
          var parameters = {
            ...
            sort:
              $scope.sort.column +
              ' ' +
              ($scope.sort.direction ? 'ASC' : 'DESC'),
            populate: ['connection'],
          };

          // Fetch data count
          var count = SnapshotSchedule.count(commonParameters).then(
          ...
          // Fetch actual data
          var load = SnapshotSchedule.load(
          ...
          SnapshotSchedule.update(data.id, {
            ...
            .then(function (updated) {
              ...
              MessageService.success(
                '计划作业' + (item.active ? '开始' : '停止')
              );
            })
            .catch(function (err) {
              console.error('Failed to update Schedule', err);
              MessageService.error('出现一些错误...', '无法更新计划');
            });
        };
