...
(function () {
  ...
  angular.module('frontend.snapshots').controller('SnapshotsListController', [
    '_',
    '$scope',
    '$rootScope',
    '$q',
    ...
    function controller(
      ...
      $scope.deleteSnapshot = function deleteSnapshot(snapshot) {
        Snapshot.delete(snapshot.id).then(function onSuccess(data) {
          MessageService.success('快照删除成功！');
          _triggerFetchData();
        });
      };
      ...
      function _fetchData() {
        ...
        // Fetch data count
        var count = Snapshot.count(commonParameters).then(function onSuccess(
          response
        ) {
      ...
      $scope.onSnapshotLoaded = function (data) {
        ...
        try {
          importedSnapshot = JSON.parse(
            $base64.decode($scope.file.data.split(',')[1])
          );
        } catch (err) {
        ...
        importedSnapshot.name =
          new Date().getTime() + '_' + importedSnapshot.name;
        ...
        Snapshot.create(importedSnapshot)
          ...
          .catch(function (err) {
            ...
            MessageService.error('无法上传快照，请确认上传的数据是否正确。');
          });
      };
