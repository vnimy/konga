...
(function () {
  ...
  angular.module('frontend.connections').controller('ConnectionsController', [
    '_',
    '$scope',
    '$rootScope',
    '$q',
    ...
    function controller(
      ...
      $scope.$watch(
        'filters',
        function watcher(valueNew, valueOld) {
          if (valueNew !== valueOld) {
            _triggerFetchData();
          }
        },
      ...
      $scope.deleteNode = function deleteNode(node) {
        if ($scope.user.node && $scope.user.node.id == node.id) {
          $ngBootbox
            .alert(
              '<p class="lead">抱歉...</p>不能删除激活的节点。<br><br>取消激活后重试'
            )
            .then(function () {});
          return false;
        }
        ...
        DialogService.confirm(
          ...
          ['否', '是'],
          function accept() {
            NodeModel.delete(node.id)
              .then(function onSuccess(data) {
                if (data.status < 204) {
                  MessageService.success('连接删除成功');
      ...
      $scope.toggleHealthChecks = function (node) {
        NodeModel.update(node.id, {
          ...
          MessageService.success(
            '该节点的健康检查' + (node.health_checks ? '已启用' : '已禁用')
          );
        });
      };
        ...
        if (!isActive(node)) {
          ...
          $http
            ...
            .catch(function (error) {
              ...
              node.checkingConnection = false;
              MessageService.error('无法连接' + node.kong_admin_url);
            });
        } else {
      ...
      $scope.updateNode = function (node) {
        NodeModel.update(node.id, node).then(
      ...
      function _fetchGatewayInfo(node) {
        InfoService.getInfo()
          .then(function (response) {
            ...
            $log.debug(
              'ConnectionsController:onUserNodeUpdated:Gateway Info =>',
              $rootScope.Gateway
            );
      ...
      function _fetchData() {
        ...
        var parameters = {
          ...
          sort:
            $scope.sort.column + ' ' + ($scope.sort.direction ? 'ASC' : 'DESC'),
        };

        // Fetch data count
        var count = NodeModel.count().then(function onSuccess(response) {
        ...
        var load = NodeModel.load(
          _.merge({}, commonParameters, parameters)
        ...
        $q.all([count, load]).finally(function onFinally() {
          $scope.loaded = true;
          $scope.loading = false;
        });
      }
      ...
      function updateActiveKongConnectionVersion() {
        ...
        if (
          activeNode &&
          activeNode.kong_version !== $rootScope.Gateway.version
        ) {
          console.log('##############################');
          ...
          console.log(
            'Active connection Kong version',
            activeNode.kong_version
          );
          console.log('##############################');

          NodeModel.update(activeNode.id, {
            kong_version: $rootScope.Gateway.version,
          }).then(
            function onSuccess(result) {
              MessageService.success('节点版本已更新');
              $rootScope.$broadcast('kong.node.updated', result.data);
            },
            function (err) {
              NodeModel.handleError($scope, err);
