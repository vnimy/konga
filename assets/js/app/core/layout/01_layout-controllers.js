...
(function () {
   ...
   *  3) Profile
   */
  angular.module('frontend.core.layout').controller('HeaderController', [
    '$scope',
    '$state',
    '$rootScope',
    ...
    function controller(
      ...
      $scope.isNotActive = function isNotActive(item) {
        return !!(
          item.state === 'examples' && $state.current.name === 'examples.about'
        );
      };
      ...
      $scope.isActive = function isActive(item) {
        ...
        return !!(item.state === $state.current.name || item.state === bits[0]);
      };
   ...
   */
  angular.module('frontend.core.layout').controller('FooterController', [
    '_',
    '$scope',
    '$state',
    'AuthService',
    ...
    function controller(
      ...
      $scope.activateConnection = function (node) {
        ...
        if (
          (UserService.user().node && UserService.user().node.id == node.id) ||
          node.checkingConnection
        ) {
          return false;
        }
        ...
        $http
          ...
          .then(function (response) {
            ...
            UserModel.update(UserService.user().id, {
              node: node,
            }).then(
              ...
              function (err) {
                ...
                MessageService.error(
                  _.get(err, 'data.message', 'Something went wrong...')
                );
              }
            );
          })
          .catch(function (error) {
            ...
            node.checkingConnection = false;
            MessageService.error(
              "Oh snap! Can't connect to " + node.kong_admin_url
            );
          });
      };
      ...
      function _fetchGatewayInfo(node) {
        InfoService.getInfo()
          .then(function (response) {
            ...
            console.log(
              'FooterController:onUserNodeUpdated:Gateway Info =>',
              $rootScope.Gateway
            );
  ...
  angular.module('frontend.core.layout').controller('SidenavController', [
    '_',
    '$scope',
    '$state',
    'AuthService',
    ...
    function controller(
      ...
      $scope.items = [
        ...
        },
        {
          ...
          show: function () {
            return (
              AuthService.hasPermission('services', 'read') &&
              $rootScope.isGatewayVersionEqOrGreater('0.13.0')
            );
          },
          title: '服务',
        ...
        },
        {
          ...
          show: function () {
            return (
              AuthService.hasPermission('routes', 'read') &&
              $rootScope.isGatewayVersionEqOrGreater('0.13.0')
            );
          },
          title: '路由',
        ...
        // },
        {
          ...
          show: function () {
            return (
              AuthService.hasPermission('consumers', 'read') &&
              $rootScope.Gateway
            );
          },
          title: '消费者',
        ...
        },
        {
          ...
          show: function () {
            return (
              AuthService.hasPermission('plugins', 'read') && $rootScope.Gateway
            );
          },
          title: '插件',
        ...
        },
        {
          ...
          show: function () {
            return (
              AuthService.hasPermission('upstreams', 'read') &&
              UserService.user().node &&
              $rootScope.isGatewayVersionEqOrGreater('0.10.0')
            );
          },
          title: '上游',
          access: AccessLevels.anon,
        },
        {
          ...
          show: function () {
            return (
              AuthService.hasPermission('certificates', 'read') &&
              UserService.user().node &&
              $rootScope.isGatewayVersionEqOrGreater('0.10.0')
            );
          },
        ...
        },
        {
          ...
          show: function () {
            return (
              ...
              AuthService.hasPermission('connections', 'read')
            );
          },
          access: AccessLevels.user,
        },
