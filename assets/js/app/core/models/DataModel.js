...
(function () {
  ...
  angular.module('frontend.core.models').factory('DataModel', [
    ...
    'DataService',
    'MessageService',
    function ($sailsSocket, $log, _, $rootScope, DataService, MessageService) {
      /**
       ...
       */
      DataModel.prototype.setScope = function setScope(
        scope,
        nameObject,
        nameObjects,
        nameCount
      ...
      DataModel.prototype.handlerUpdated = function handlerUpdated(message) {
        ...
        if (self.scope) {
          if (
            self.itemNames.object &&
            parseInt(message.id, 10) === parseInt(self.object.id, 10)
          ) {
       ...
       *         }}  message
       */
      DataModel.prototype.handlerRemovedFrom = function handlerRemovedFrom(
        message
      ) {
        var self = this;
      ...
      DataModel.prototype.count = function count(parameters, fromCache) {
        ...
        return DataService.count(self.endpoint, parameters).then(
      ...
      DataModel.prototype.load = function load(parameters, fromCache) {
        ...
        return DataService.collection(self.endpoint, parameters).then(
          function onSuccess(response) {
          ...
          function onError(error) {
            $log.error(
              'DataModel.load() failed.',
              error,
              self.endpoint,
              parameters
            );
            MessageService.error(
              _.get(error, 'data.body.message', '出现意外错误...')
            );
          }
        );
      };
      ...
      DataModel.prototype.fetch = function fetch(
        ...
        return DataService.fetch(self.endpoint, identifier, parameters).then(
          function onSuccess(response) {
      ...
      DataModel.prototype.updateOrCreate = function updateOrCreate(data) {
        ...
        return DataService.updateOrCreate(self.endpoint, data);
      };
      ...
      DataModel.prototype._subscribe = function subscribe() {
        ...
        $sailsSocket.subscribe(
          self.endpoint.replace('api/', ''),
          function modelEvent(message) {
            // Handle socket event
      ...
      DataModel.prototype._handleEvent = function handleEvent(message) {
        var self = this;
        var method =
          'handler' + message.verb[0].toUpperCase() + message.verb.slice(1);
        ...
        if (_.isFunction(self[method])) {
          ...
          $log.log(
            "Implement handling for '" + message.verb + "' socket messages"
          );
        }
      };
