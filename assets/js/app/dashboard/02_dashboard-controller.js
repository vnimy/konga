...
(function () {
  ...
  angular.module('frontend.dashboard').controller('DashboardController', [
    '$scope',
    '$rootScope',
    '$log',
    '$state',
    ...
    function controller(
      ...
      $scope.showCluster = $rootScope.Gateway
      ...
      function drawCharts() {
        $scope.data = {
          ...
          timers: {
            ...
            series: ['计时器'],
            data: [$scope.info.timers.pending, $scope.info.timers.running],
          },
          database: {
      ...
      function fetchData() {
        ...
        var status = InfoService.nodeStatus().then(function (resp) {
          $scope.status = resp.data;
          $log.info('DashboardController:fetchData:status', $scope.status);
        });
        var info = InfoService.getInfo().then(function (resp) {
          $scope.info = resp.data;
          $log.info('DashboardController:fetchData:info', $scope.info);
        });

        $q.all([status, info]).finally(function onFinally() {
          $scope.loading = false;
      ...
      $scope.create = function () {
        ...
        NodeModel.create(angular.copy($scope.node)).then(
            ...
            $http
              ...
              .then(function (response) {
                ...
                if (window.no_auth) {
                  ...
                  if (
                    credentials.user.node &&
                    credentials.user.node.id == node.id
                  ) {
              ...
              .catch(function (error) {
                ...
                $scope.busy = false;
                MessageService.error(
                  "Oh snap! Can't connect to the created node. Check your connections."
                );
              });
          },
          function (err) {
