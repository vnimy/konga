...
(function () {
  ...
  angular.module('frontend.users').controller('UserController', [
    '_',
    '$scope',
    '$q',
    '$log',
    ...
    function controller(
      ...
      $scope.saveUser = function saveUser() {
        ...
        UserModel.update(data.id, data).then(
          function onSuccess(data) {
            $scope.showForm = false;
            MessageService.success(
              '用户"' + $scope.user.username + '"更新成功'
            );
            initUserPassports();
      ...
      $scope.deleteUser = function deleteUser() {
        UserModel.delete($scope.user.id)
          .then(function onSuccess() {
            MessageService.success(
              '用户"' + $scope.user.username + '"删除成功'
            );
            $state.go('users');
          })
          .catch((err) => {
            MessageService.error(_.get(err, 'data.message', '删除用户失败'));
          });
      };
      ...
      function fetchConnections() {
        NodeModel.load().then(function onSuccess(response) {
