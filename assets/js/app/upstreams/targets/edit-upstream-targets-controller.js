  ...
  angular
    ...
    .controller('EditUpstreamTargetsController', [
      ...
      function controller(
        ...
        var targetsEndpoint =
          $rootScope.compareKongVersion('0.12.0') >= 0
            ? '/targets'
            : '/targets/active';
        var Target = new DataModel(
          'kong/upstreams/' + $stateParams.id + targetsEndpoint,
          true
        );
        ...
        $scope = angular.extend(
          $scope,
          angular.copy(ListConfig.getConfig('target', Target))
        );
          ...
          var modalInstance = $uibModal.open({
            ...
            controller: [
              ...
              function (
                ...
                var targetModel = new DataModel(
                  'kong/upstreams/' + _upstream.id + '/targets',
                  true
                );
                ...
                $scope.submit = function () {
                  targetModel.create($scope.item).then(
                    function (resp) {
                      $log.debug('Create target =>', resp);
                      MessageService.success('后端服务器添加成功！');
                      $uibModalInstance.dismiss({
                        data: resp,
                      });
                    },
        ...
        $scope.deleteItem = function (index, item) {
          DialogService.confirm(
            ...
            function accept() {
              $http
                .delete(
                  'kong/upstreams/' + $stateParams.id + '/targets/' + item.id
                )
        ...
        function setHealth(index, target, healthy) {
          ...
          $http
            .post(
              `kong/upstreams/${$stateParams.id}/targets/${target.id}/${
                healthy ? 'healthy' : 'unhealthy'
              }`
            )
            .then((res) => {
              MessageService.success(
                `后端服务器“${target.target}”被设置为${
                  healthy ? '健康' : '不健康'
                }状态`
              );
