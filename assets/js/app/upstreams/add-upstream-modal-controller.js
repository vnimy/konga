      ...
      function controller(
        ...
        $scope.onTagInputKeyPress = function ($event) {
          if ($event.keyCode === 13) {
            ...
            $scope.upstream.tags = $scope.upstream.tags.concat(
        ...
        $scope.submit = function () {
          ...
          arr.forEach(function (item) {
            if (_.get(data, 'healthchecks.' + item + '.http_statuses')) {
              _.update(
                data,
                'healthchecks.' + item + '.http_statuses',
                function (statuses) {
          ...
          Upstream.create(data).then(
            ...
            function (err) {
              $log.error(
                'AddUpstreamModalController:created upstream error',
                err
              );
              $scope.busy = false;
