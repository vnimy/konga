    ...
    .controller('ServiceHealthChecksController', [
      ...
      function controller(
        ...
        $scope.toggleServiceHC = function () {
          ServiceHCModel.update($scope.serviceHC.id, {
            active: $scope.serviceHC.active,
          }).then(
            function (updated) {
              MessageService.success(
                '健康检查' + ($scope.serviceHC.active ? '已启用' : '已禁用')
              );
            },
            function (err) {
              //
        ...
        $scope.save = function () {
          ...
          ServiceHCModel.update($scope.serviceHC.id, $scope.serviceHC).then(
            function (updated) {
              MessageService.success('服务健康检查已更新！');
            },
            function (err) {
              //
            }
