...
(function () {
  ...
  angular
    .module('frontend.consumers.groups')
    .controller('ManageKongGroupController', [
      ...
      function controller(
            ...
            controller: [
              ...
              function ($scope, $rootScope, $uibModalInstance, KongGroupModel) {
                ...
                function createGroup() {
                  KongGroupModel.create(angular.copy($scope.group)).then(
                    function onSuccess(result) {
                      if (result.data && result.data.error) {
                        $scope.errors = {};
                        for (var key in result.data.invalidAttributes) {
                          $scope.errors[key] =
                            result.data.invalidAttributes[key][0].message;
                        }
                      } else {
                        MessageService.success('分组创建成功！');
                    ...
                    function onError(err) {
                      $log.error(err);
                      $scope.errors =
                        err.data.body || err.data.customMessage || {};
                    }
                  );
                }
        ...
        function updateGroup(group) {
          KongGroupModel.update(group.id, group).then(
            function onSuccess(result) {
        ...
        function deleteGroup(group) {
          DialogService.confirm(
            ...
            ['否', '删除'],
            function accept() {
              KongGroupModel.delete(group.id).then(function onSuccess(result) {
                MessageService.success('分组删除成功。');
        ...
        function fetchGroups() {
          KongGroupModel.load().then(function (data) {
            $scope.groups = data;
          });
        }
