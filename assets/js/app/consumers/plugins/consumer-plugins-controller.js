...
(function () {
  ...
  angular.module('frontend.consumers').controller('ConsumerPluginsController', [
    '_',
    '$scope',
    '$stateParams',
    '$log',
    ...
    function controller(
      ...
      function loadEntity(item, entity, id) {
        item.loading = true;
        $http
          .get(BackendConfig.url + `kong/${entity}s/${id}`)
          .then((result) => {
            // item.loading = false;
        ...
        DialogService.confirm(
          ...
          function accept() {
            PluginsService.delete(plugin.id)
              .then(function (resp) {
                $scope.plugins.data.splice(
                  $scope.plugins.data.indexOf(plugin),
                  1
                );
              })
              .catch(function (err) {
      ...
      function fetchPlugins() {
        ConsumerService.listPlugins($stateParams.id).then(function (res) {
