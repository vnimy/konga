...
(function () {
  ...
  angular.module('frontend.plugins').controller('AddPluginController', [
    '_',
    '$scope',
    '$rootScope',
    '$log',
    ...
    function controller(
      ...
      if (_.isArray(sch.fields)) {
        sch.fields.forEach((item) => {
          $scope.schema.fields[Object.keys(item)[0]] =
      ...
      function initialize() {
        ...
        $scope.description = $scope.data.meta
          ? $scope.data.meta.description
          : '配置插件。';
        ...
        PluginHelperService.customizeDataFieldsForPlugin(
          _pluginName,
          $scope.data.fields
        );

        // Assign extra properties from options to data fields
        PluginHelperService.assignExtraProperties(
          _pluginName,
          options,
          $scope.data.fields
        );
      ...
      $scope.addPlugin = function (back) {
        ...
        PluginHelperService.applyMonkeyPatches(
          request_data,
          $scope.data.fields
        );

        // Create request data "config." properties
        var config = PluginHelperService.createConfigProperties(
          request_data.name,
          $scope.data.fields
        );
        ...
        if (request_data.config) {
          Object.keys(request_data.config).forEach(function (key) {
            if (
              !request_data.config[key] ||
              (Array.isArray(request_data.config[key]) &&
                !request_data.config[key].length) ||
      ...
      $scope.getConsumer = function (val) {
        ...
        return ConsumerModel.load(
          _.merge({}, commonParameters, {
            limit: 5,
          })
      ...
      $scope.statsd = {
        ...
        statTypes: ['gauge', 'timer', 'counter', 'histogram', 'meter', 'set'],
        consumerIndentifiers: ['consumer_id', 'custom_id', 'username'],
      ...
      $scope.addMap = (obj) => {
        DialogService.prompt('添加限制', 'Enter key', (keyName) => {
          if (!obj.value) {
            obj.value = {};
          }
