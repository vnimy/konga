...
(function () {
  ...
  angular.module('frontend.plugins').controller('EditPluginController', [
    '_',
    '$scope',
    '$rootScope',
    '$log',
    ...
    function controller(
      ...
      function assignValues(fields, prefix) {
        Object.keys(fields).forEach(function (item) {
          if (fields[item].schema) {
            ...
            fields[item].help = _.get(options, path)
              ? _.get(options, path).help
              : '';
          }
        });
      }
      ...
      $scope.updatePlugin = function () {
        ...
        const config = PluginHelperService.createConfigProperties(
          _plugin.name,
          $scope.data.fields
        );
        ...
        PluginsService.update(_plugin.id, finalData)
          .then(function (res) {
            ...
            MessageService.success('"' + _plugin.name + '"插件更新成功！');
          ...
          .catch(function (err) {
            ...
            if (err.data.customMessage) {
              Object.keys(err.data.customMessage).forEach(function (key) {
                errors[key.replace('config.', '')] =
                  err.data.customMessage[key];
